...
HealthDisplayTime(4.0f),
HitReactTimeMin(0.5f), HitReactTimeMax(3.0f), bCanHitReact(true), 
...
bCanAttack(true), AttackWaitTime(1.0f),
bDying(false), DeathTime(10.0f)
{
...
void AEnemy::BeginPlay()
{
	...
	LeftWeaponCollision->SetCollisionResponseToChannels(ECollisionResponse::ECR_Ignore);
...
void AEnemy::ShowHealthBar_Implementation()
{
	...
	GetWorldTimerManager().SetTimer(HealthBarTimer, this, &AEnemy::HideHealthBar, HealthDisplayTime);
}
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	auto Charapter = Cast<AZHeroCharacter>(OtherActor);
	if(Charapter)
	{
		if(EnemyController)
		{
			...
			{
				EnemyController->GetBlackboardComponent()->SetValueAsObject(TEXT("Target"), Charapter);
			}
		}
	}
}
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	auto HeroCharapter = Cast<AZHeroCharacter>(OtherActor);
	if(HeroCharapter)
	{
		bInAttackRange = true;
		if(EnemyController)
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		DoDamage(Charapter);
		SpawnBlood(Charapter, LeftWeaponSocket);
		StunCharapter(Charapter);
	}
}
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		DoDamage(Charapter);
		SpawnBlood(Charapter, RightWeaponSocket);
		StunCharapter(Charapter);
	}
}
...
void AEnemy::DoDamage(AZHeroCharacter* Victim)
{
...
void AEnemy::SpawnBlood(AZHeroCharacter* Victim, FName SocketName)
{
...
void AEnemy::StunCharapter(AZHeroCharacter* Victim)
{
	if(Victim)
	{
